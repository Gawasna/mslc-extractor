...
static std::wstring g_customPipeName = L"\\\\.\\pipe\\LiveCaptionPipeResearch";
            ...
            if (dir.find(L"x64\\Release") != std::wstring::npos || dir.find(L"x64\\Debug") != std::wstring::npos) {
                ...
                if (pos != std::wstring::npos) {
                    ...
                    if (pos != std::wstring::npos) {
                        ...
                        std::wstring logFile = projectRoot + L"\\logs\\mslc_host_research.log";
                        return std::string(logFile.begin(), logFile.end());
                    }
                }
            } else {
                std::wstring logFile = dir + L"\\logs\\mslc_host_research.log";
                return std::string(logFile.begin(), logFile.end());
            }
        }
    }
    return "C:\\Users\\Public\\mslc_host_research.log"; // Fallback
}
...
bool ParseResearchEvent(const std::wstring& data, ResearchEvent& out) {
    size_t p = data.find(L"\"event\":\"");
    if (p == std::wstring::npos) return false;
    p += 9;
    size_t e = data.find(L'"', p);
    if (e == std::wstring::npos) return false;
    out.event_type = data.substr(p, e - p);

    p = data.find(L"\"precise_ticks\":");
    if (p != std::wstring::npos) {
        out.precise_ticks = static_cast<DWORD64>(_wtoi64(data.c_str() + p + 16));
    }
    ...
    if (p != std::wstring::npos) {
        p += 9;
        size_t end = data.find(L'"', p);
        if (end != std::wstring::npos) {
            out.hreco = data.substr(p, end - p);
        }
    }

    p = data.find(L"\"hevent\":\"");
    if (p != std::wstring::npos) {
        p += 10;
        size_t end = data.find(L'"', p);
        if (end != std::wstring::npos) {
            out.hevent = data.substr(p, end - p);
        }
    }

    p = data.find(L"\"hresult\":\"");
    if (p != std::wstring::npos) {
        p += 11;
        size_t end = data.find(L'"', p);
        if (end != std::wstring::npos) {
            out.hresult = data.substr(p, end - p);
        }
    }
        ...
        if (pos != std::wstring::npos) {
            ...
            if (dir.find(L"x64\\Release") != std::wstring::npos || dir.find(L"x64\\Debug") != std::wstring::npos) {
                ...
                if (pos != std::wstring::npos) {
                    ...
                    if (pos != std::wstring::npos) {
                        std::wstring projectRoot = root.substr(0, pos);
                        std::wstring logFile = projectRoot + L"\\logs\\mslc_research_analysis.log";
                        return std::string(logFile.begin(), logFile.end());
                    }
                }
            } else {
                std::wstring logFile = dir + L"\\logs\\mslc_research_analysis.log";
                return std::string(logFile.begin(), logFile.end());
            }
        }
    }
    return "C:\\Users\\Public\\mslc_research_analysis.log"; // Fallback
}

void LogResearchRaw(const std::string& jsonData, DWORD64 recvTick) {
    SYSTEMTIME st;
    GetLocalTime(&st);
    char timeStr[64];
    sprintf_s(timeStr, "[%04d-%02d-%02d %02d:%02d:%02d.%03d] [RECV:%llu] ", 
              st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, recvTick);

    std::string rLogPath = GetResearchLogPath();
    std::ofstream f(rLogPath, std::ios_base::app);
    if (f.is_open()) {
        f << timeStr << jsonData << "\n";
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (!g_exitHost) {
        ...
        if (!g_mockMode) {
            ...
            std::wstring dllPath = strPath.substr(0, strPath.find_last_of(L"\\")) + L"\\AgentResearch.dll";
            ...
            while (!g_exitHost) {
                ...
                if (pid != 0) {
                    ...
                    if (IsDLLAlreadyInjected(pid, L"AgentResearch.dll")) {
                        if (!g_stdoutOnly && !g_injectOnly) {
                            std::cout << "[+] AgentResearch.dll already injected. Listening..." << std::endl;
                        }
                        g_targetPid = pid;
                        break;
                    }
                    ...
                    if (InjectDLL(pid, dllPath)) {
                        if (!g_stdoutOnly && !g_injectOnly) {
                            std::cout << "[+] AgentResearch.dll injected successfully. Listening..." << std::endl;
                        }
                        g_targetPid = pid;
                        break;
                    } else {
        ...
        while (!g_exitHost && !g_needReinjection) {
            ...
            // Log raw research packets
            LogResearchRaw(rawPkt.data, rawPkt.recvTick);
            ...
            ResearchEvent rEvt;
            if (ParseResearchEvent(wideBuf.data(), rEvt)) {
                if (rEvt.event_type != L"caption") {
                    std::cout << "[RESEARCH_EVENT] " << WideToUTF8(rEvt.event_type)
                              << " precise_ticks=" << rEvt.precise_ticks
                              << " ts_ms=" << rEvt.ts_ms;
                    if (!rEvt.hreco.empty()) std::cout << " hreco=" << WideToUTF8(rEvt.hreco);
                    if (!rEvt.hevent.empty()) std::cout << " hevent=" << WideToUTF8(rEvt.hevent);
                    if (!rEvt.hresult.empty()) std::cout << " hresult=" << WideToUTF8(rEvt.hresult);
                    if (rEvt.offset > 0) std::cout << " offset=" << rEvt.offset;
                    if (rEvt.duration > 0) std::cout << " duration=" << rEvt.duration;
                    std::cout << std::endl;
                    continue;
                }
            }
